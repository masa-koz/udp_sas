...
fn getsockopt<T>(
    ...
    #[cfg(target_family = "windows")]
    unsafe {
        let mut len = std::mem::size_of::<T>() as winapi::um::ws2tcpip::socklen_t;
        try_io!(winapi::um::winsock2::getsockopt(
            socket,
            level,
            name,
            value as *mut T as *mut winapi::ctypes::c_char,
            &mut len
        ));
        Ok(len)
    }
...
fn setsockopt<T>(
    ...
    #[cfg(target_family = "windows")]
    unsafe {
        try_io!(winapi::um::winsock2::setsockopt(
            socket,
            level,
            name,
            value as *const T as *mut winapi::ctypes::c_char,
            std::mem::size_of::<T>() as winapi::um::ws2tcpip::socklen_t
        ));
        Ok(())
    }
}
...
pub fn set_pktinfo(socket: SOCKET, family: libc::c_int) -> io::Result<()> {
    unsafe {
        let (level, option) = match family {
            AF_INET => (IPPROTO_IP, IP_PKTINFO),
            AF_INET6 => (IPPROTO_IPV6, IPV6_RECVPKTINFO),
            _ => {
                return Err(io::Error::new(io::ErrorKind::Other, "not an inet socket"));
            }
        };

        setsockopt(socket, level as libc::c_int, option, &(1 as libc::c_int))
    }
}
    ...
    let nb = {
        unsafe {
            udp_sas_recv(
                ...
                0,
                src.as_mut_ptr(),
                src.capacity() as socklen_t,
                dst.as_mut_ptr(),
                dst.capacity() as socklen_t,
            )
        }
    };
...
pub fn send_sas(
    ...
    let nb = unsafe {
        udp_sas_send(
            ...
            0,
            src.as_ptr(),
            src.len() as socklen_t,
            dst.as_ptr(),
            dst.len() as socklen_t,
        )
    };
    if nb < 0 {
        Err(io::Error::last_os_error())
